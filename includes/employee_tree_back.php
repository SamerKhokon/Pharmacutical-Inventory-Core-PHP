...
function edit_data(id)
{
	...
	$.ajax({
		type:"post" ,
		url:"includes/menu_info_for_edit.php" ,
		data:dataStr ,
		cache:false ,
		success:function(st)
		{
...
<ul id="tree">
    <?php
         $string = "SELECT distinct superior_emp_id,employee_id,(SELECT emp_name FROM `hr_employee_info` WHERE employee_id=a.superior_emp_id)  AS emp_name
				FROM `hr_employee_hierarchy` AS a";
		 $query = mysql_query($string);
		 while($rs = mysql_fetch_assoc($query) ){
		    $emp_id = $rs['employee_id'];
	?>
	<li><a href="javascript:void(0);"><strong><?php echo $rs['emp_name'];?></strong></a>
	<ul>
	   <?php
            $subString = "SELECT emp_name employee_name FROM `hr_employee_info` WHERE employee_id=$emp_id";
			$subQuery = mysql_query($subString);
			while($res = mysql_fetch_assoc($subQuery)) {
	   ?>
		<li><a href="javascript:void(0);"><?php echo $res['employee_name'];?></a></li>
	  <?php } ?>	
	</ul>
	</li>
	<?php } ?>
