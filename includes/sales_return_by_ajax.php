<?php  include("../db.php");
	 ...
	 $mr_id 								 = 0;
	 $sr_id 								 = 0;
	 $invoice_no 						 = trim($_POST["inv_id"]);
	 $invoice_status = trim($_POST['invoice_status']);
	 $reference_no 					 = '-';
	 $delivery_date 					 = trim($_POST["return_date"]);
	 $order_ids 							 = '-';
	 $depot_id = get_customer_depot_id($customer_id);
	 $prev_due 						     = get_customer_current_due($customer_id,$depot_id);
	 
	 $total_due 							 = trim($_POST["grand_total"]);
	 $net_invoice_amount 			 = trim($_POST["grand_total"]);
	 $discount_percentage            = trim($_POST['com_amnt_per']);
	 $discount = trim($_POST['com_amnt']);
	 
	 
	 $total_unitPrice  = trim($_POST["sales_amnt"]);
	 
	 if($discount_percentage=='')
	 	$discount_percentage = 0;
	 $discount_amount 				 = trim($_POST["com_amnt"]);
	 if($discount_amount=='')
	 	$discount_amount = 0;
	 ...
	 /*************************************************************************************
	   List arrays
	 **************************************************************************************/
	 $product_id_list 					= trim($_POST["products"]);
	 $pack_size_list 					= trim($_POST["packs"]);
	 $unit_list 							= trim($_POST["units"]);
	 $quantity_list 						= trim($_POST["qtys"]);
	 //$bonus_offer_list 				= trim($_POST["bonus_offer_list"]);
	 $bonus_qty_list 					= trim($_POST["bqtys"]);
	 //$total_price_list 					= trim($_POST["total_price_list"]);
	 $unit_price_list 					= trim($_POST["uprices"]);
	...
	`discount_amount`, `payed_ammount`, `current_invoice_due`, `net_invoice_amount`,`total_unit_price`,`invoice_status`)
	VALUES	($customer_id, '$customer_code', '$order_nos',	'$order_ids',
	$mr_id,'$mr_code', $sr_id, '$sr_code', '$invoice_no','$reference_no', str_to_date('$delivery_date','%m/%d/%Y'),$entry_by, 
	NOW(), $company_id, 'Y', '$chk_data','$prev_due',$discount,$discount_amount,0,
	$net_invoice_amount,  $net_invoice_amount,$total_unitPrice,'$invoice_status')";
