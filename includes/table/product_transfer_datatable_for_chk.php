<?php	include("../../db.php");  
		...
		*/
		$transfer_from = $_POST['transfer_from_depot_id'];
		$transfer_to = $_POST['transfer_to_depot_id'];
		$company_id = $_POST['company_id'];
		
	   	$str = "SELECT 
			a.challan_no as challan_no,
			a.invoice_no as invoice_no,
			a.ref_no as ref_no,
			a.transfer_from as transfer_from,
			(SELECT depot_name FROM `inv_depot_info` WHERE depot_id=a.transfer_from) AS transfer_from_depo,
			a.transfer_from_code as transfer_from_code,
			a.transfer_to as transfer_to,
			(SELECT depot_name FROM `inv_depot_info` WHERE depot_id=a.transfer_to) AS  transfer_to_depo,
			a.transfer_to_code as transfer_to_code,
			a.transport_id as transport_id,
			(SELECT vehicle_name FROM `vehicle_info` WHERE vehicle_id=a.transport_id) AS  transport_name,
			a.transport_type as transport_type,
			a.load_supervised_by as load_supervised_by,
			(SELECT emp_name FROM `hr_employee_info` WHERE employee_id=a.load_supervised_by) AS supervisor_name,
			a.in_charge_emp_id as in_charge_emp_id,
			(SELECT emp_name FROM `hr_employee_info` WHERE employee_id=a.in_charge_emp_id) AS in_charge_name,
			date_format(a.delivery_date,'%d-%m-%Y') as delivery_date,
			a.process_status as process_status,
			b.ddpt_id as ddpt_id,
			b.product_id as product_id,
			(SELECT product_name FROM product_info WHERE product_id=b.product_id) AS product_name,
			b.product_code as product_code,
			b.pac_size_id as pac_size_id,
			b.pack_size as pack_size,
			b.product_unit_id as product_unit_id,
			b.product_unit as product_unit,
			b.product_unit_id as product_unit_id,
			b.product_unit as product_unit,
			b.product_quantity as product_quantity,
			b.product_bonus_quantity as product_bonus_quantity,
			b.unit_price as unit_price,
			b.total_unit_price as total_unit_price
		FROM 
			`product_transfer_info` a,
			`transfering_product_info` b 
		WHERE a.company_id=$company_id and a.transfer_from=$transfer_from and transfer_to=$transfer_to";
	   	$sql = mysql_query($str);
